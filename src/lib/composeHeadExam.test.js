...
describe('composeHeadExam', () => {
  it('should correctly compose normal head exam bullet', () => {
    const params = {
      findingsWithInput: headFindings.map(f => ({
        ...f,
        verifiedFindings: normalHeadInputs.filter(vf => vf.findingCode === f.code)
      })),
    };
    const section = composeHeadExam(params);
    expect(section).to.deep.equal({
      subtitle: 'Head',
      body: 'normalocephalic, atraumatic',
    });
  });
  ...
  it('should correctly compose abnormal head exam bullet', () => {
    const params = {
      findingsWithInput: headFindings.map(f => ({
        ...f,
        verifiedFindings: abnormalHeadInputs.filter(vf => vf.findingCode === f.code)
      }))
    };
    const section = composeHeadExam(params);
    expect(section).to.deep.equal({
      subtitle: 'Head',
      body: 'abnormal head shape, head injury',
    });
  });
});
